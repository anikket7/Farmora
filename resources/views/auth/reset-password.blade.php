...
<head>
    ...
    <title>Reset Password - Farmora</title>
        ...
        <div class="w-full max-w-4xl grid md:grid-cols-2 gap-0 rounded-xl overflow-hidden shadow-[0_16px_64px_-12px_rgba(203,163,88,0.1)] border border-white/10 backdrop-blur-3xl bg-surface-container/30">
            ...
            <div class="p-card-padding md:p-8 flex flex-col h-full bg-surface-container/20">
                <div class="mb-8 text-center md:text-left">
                    <h2 class="font-headline-md text-headline-md text-on-surface mb-2">Set New Password</h2>
                    <p class="font-body-md text-body-md text-on-surface-variant">Update your account password securely.</p>
                </div>

                <form method="POST" action="{{ route('password.update') }}" class="flex flex-col gap-6 grow">
                    @csrf
                    ...
                    <div class="flex flex-col gap-2">
                        <label for="token" class="font-label-caps text-label-caps text-on-surface-variant pl-4">5-Digit OTP Code</label>
                        <div class="relative">
                            <span class="material-symbols-outlined absolute left-4 top-1/2 -translate-y-1/2 text-on-surface-variant" style="font-variation-settings: 'FILL' 0;">key</span>
                            <input id="token" name="token" value="{{ old('token') }}" required autofocus class="w-full bg-surface-container-low border border-white/10 rounded-full py-3 pl-12 pr-4 font-body-md text-body-md text-on-surface focus:outline-none focus:border-secondary-fixed focus:ring-1 focus:ring-secondary-fixed transition-all placeholder:text-on-surface-variant/50" placeholder="Enter 5-digit OTP" type="text" maxlength="5"/>
                        </div>
                        @error('token')
                            <p class="text-error text-sm mt-1 pl-4">{{ $message }}</p>
                        @enderror
                    ...
                    <div class="flex flex-col gap-2">
                        <label for="password" class="font-label-caps text-label-caps text-on-surface-variant pl-4">New Password</label>
                        <div class="relative">
                            ...
                            <input id="password" name="password" required class="w-full bg-surface-container-low border border-white/10 rounded-full py-3 pl-12 pr-4 font-body-md text-body-md text-on-surface focus:outline-none focus:border-secondary-fixed focus:ring-1 focus:ring-secondary-fixed transition-all placeholder:text-on-surface-variant/50" placeholder="Min 8 characters" type="password"/>
                        </div>
                        @error('password')
                            <p class="text-error text-sm mt-1 pl-4">{{ $message }}</p>
                        @enderror
                    ...
                    <div class="flex flex-col gap-2">
                        <label for="password_confirmation" class="font-label-caps text-label-caps text-on-surface-variant pl-4">Confirm New Password</label>
                        <div class="relative">
                            <span class="material-symbols-outlined absolute left-4 top-1/2 -translate-y-1/2 text-on-surface-variant" style="font-variation-settings: 'FILL' 0;">lock</span>
                            <input id="password_confirmation" name="password_confirmation" required class="w-full bg-surface-container-low border border-white/10 rounded-full py-3 pl-12 pr-4 font-body-md text-body-md text-on-surface focus:outline-none focus:border-secondary-fixed focus:ring-1 focus:ring-secondary-fixed transition-all placeholder:text-on-surface-variant/50" placeholder="••••••••" type="password"/>
                        </div>
                    </div>
